    ...
    while (*p)
    {
        switch (*p)
        {
            ...
            case '|':
                ...
                break;
            case '\0':
                return q - outbuf;  /* redundant & not used */
        }
        if (rightward - 4 < 0)
        {
            *(q + rightward) = '\0';
...
static int pr_args(char *restrict const outbuf, const proc_t *restrict const pp)
{
    ...
    {
        flags = ESC_DEFUNCT | ESC_BRACKETS | ESC_ARGS;
    }
...
static int pr_c(char *restrict const outbuf, const proc_t *restrict const pp)
{
    ...
    if (seconds)
    {
        pcpu = (total_time * 100ULL / Hertz) / seconds;
    }
    if (pcpu > 99U)
    {
        pcpu = 99U;
...
static int pr_class(char *restrict const outbuf, const proc_t *restrict const pp)
{
    ...
    {
        case -1:
            return snprintf(outbuf, COLWID, "-");   // not reported
        case  0:
            return snprintf(outbuf, COLWID, "TS");  // SCHED_OTHER SCHED_NORMAL
        case  1:
            return snprintf(outbuf, COLWID, "FF");  // SCHED_FIFO
        case  2:
            return snprintf(outbuf, COLWID, "RR");  // SCHED_RR
        case  3:
            return snprintf(outbuf, COLWID, "B");   // SCHED_BATCH
        case  4:
            return snprintf(outbuf, COLWID, "ISO"); // reserved for SCHED_ISO (Con Kolivas)
        case  5:
            return snprintf(outbuf, COLWID, "IDL"); // SCHED_IDLE
        case  6:
            return snprintf(outbuf, COLWID, "#6");  //
        case  7:
            return snprintf(outbuf, COLWID, "#7");  //
        case  8:
            return snprintf(outbuf, COLWID, "#8");  //
        case  9:
            return snprintf(outbuf, COLWID, "#9");  //
        default:
            return snprintf(outbuf, COLWID, "?");   // unknown value
    }
}
...
static int pr_wchan(char *restrict const outbuf, const proc_t *restrict const pp)
{
    ...
    const char *w;
    size_t len;
    if (!(pp->wchan & 0xffffff))
    {
        return memcpy(outbuf, "-", 2), 1;
    }
    if (wchan_is_number)
    {
...
static int pr_stat(char *restrict const outbuf, const proc_t *restrict const pp)
{
    ...
    if (pp->session == pp->tgid)
    {
        outbuf[end++] = 's';    // session leader
    }
    if (pp->nlwp > 1)
    {
        outbuf[end++] = 'l';    // multi-threaded
    }
    if (pp->pgrp == pp->tpgid)
    {
        outbuf[end++] = '+';    // in foreground process group
    }
    outbuf[end] = '\0';
    return end;
}
...
static int old_time_helper(char *dst, unsigned long long t, unsigned long long rel)
{
    if (!t)
    {
        return snprintf(dst, COLWID, "    -");
    }
    if (t == ~0ULL)
    {
        return snprintf(dst, COLWID, "   xx");
    }
    if ((long long)(t -= rel) < 0)
    {
        t = 0ULL;
    }
    if (t > 9999ULL)
    {
        return snprintf(dst, COLWID, "%5Lu", t / 100ULL);
    }
    else
    {
        return snprintf(dst, COLWID, "%2u.%02u", (unsigned)t / 100U, (unsigned)t % 100U);
    }
}
...
static int pr_bsdstart(char *restrict const outbuf, const proc_t *restrict const pp)
{
    ...
    {
        strcpy(outbuf, ctime(&start) + 4);
    }
    else
    {
        strcpy(outbuf, ctime(&start) + 10);
    }
    outbuf[6] = '\0';
    return 6;
}
...
static int pr_dsiz(char *restrict const outbuf, const proc_t *restrict const pp)
{
    ...
    if (pp->vsize)
    {
        dsiz += (pp->vsize - pp->end_code + pp->start_code) >> 10;
    }
...
static int pr_tsiz(char *restrict const outbuf, const proc_t *restrict const pp)
{
    ...
    if (pp->vsize)
    {
        tsiz += (pp->end_code - pp->start_code) >> 10;
    }
...
static int pr_drs(char *restrict const outbuf, const proc_t *restrict const pp)
{
    ...
    if (pp->vsize)
    {
        drs += (pp->vsize - pp->end_code + pp->start_code) >> 10;
    }
...
static int pr_trs(char *restrict const outbuf, const proc_t *restrict const pp)
{
    ...
    if (pp->vsize)
    {
        trs += (pp->end_code - pp->start_code) >> 10;
    }
...
static int help_pr_sig(char *restrict const outbuf, const char *restrict const sig)
{
    ...
    if (wide_signals)
    {
        if (len > 8)
        {
            return snprintf(outbuf, COLWID, "%s", sig);
        }
...
static int help_pr_sig(unsigned long long sig)
{
    if (wide_signals)
    {
        return snprintf(outbuf, COLWID, "%016Lx", sig);
    }
    if (sig >> 32)
    {
...
static void check_header_width(void)
{
    ...
    for (;;)
    {
        ...
        if (active_cols >= total)
        {
            break;
        }
        if (screen_cols * i >= OUTBUF_SIZE / 2)
        {
            break;    /* can't go over */
        }
    }
...
void show_one_proc(const proc_t *restrict const p, const format_node *restrict fmt)
{
    ...
    did_stuff = 1;
    if (unlikely(active_cols > (int)OUTBUF_SIZE))
    {
        fprintf(stderr, "Fix bigness error.\n");
    }
    ...
    for (;;)
    {
        ...
        if (likely(p) && likely(fmt->pr))
        {
            amount = (*fmt->pr)(outbuf, p);
        }
        else
        {
            amount = strlen(strcpy(outbuf, fmt->name));    /* AIX or headers */
        }
        ...
        if (unlikely(space > SPACE_AMOUNT))
        {
            space = SPACE_AMOUNT;    // only so much available
        }
