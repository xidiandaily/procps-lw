...
static void check_proc(int pid)
{
    ...
    {
        i = uid_count;
        while (i--) if (uids[i] == statbuf.st_uid)
            {
                break;
            }
        if (i == -1)
    ...
    *tmp++ = '\0';
    i = 5;
    while (i--) while (*tmp++ != ' '); /* scan to find tty */
    tty = atoi(tmp);
    if (ttys)
    {
        i = tty_count;
        while (i--) if (ttys[i] == tty)
            {
                break;
            }
        if (i == -1)
    ...
    if (cmds)
    {
        ...
        while (i--) if (cmds[i][0] == *tmp && !strcmp(cmds[i], tmp))
            {
                break;
            }
        if (i == -1)
        ...
        switch (force) /* fall through each data type */
        {
            ...
            case 't':
                if (argc)
                {
                    ...
                    if (!argptr)
                    {
                        skillsnice_usage();    /* Huh? Maybe "skill -t ''". */
                    }
            ...
            case 'p':
                ...
                if (num_found)
                {
                    continue;    /* could still be an option */
                }
            case 'c':
                if (argc)
                {
...
int main(int argc, const char *argv[])
{
    ...
    if (*tmpstr == 's')
    {
        ...
        if (!strcmp(tmpstr, "snice"))
        {
            program = PROG_SNICE;
        }
        if (!strcmp(tmpstr, "skill"))
        {
            program = PROG_SKILL;
        }
    }
