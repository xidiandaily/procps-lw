...
static format_node *do_one_spec(const char *spec, const char *override)
{
    ...
    if (fs)
    {
        ...
        if (fs->flags & CF_PIDMAX)
        {
            ...
            if (w2 > w1)
            {
                w1 = w2;    // FIXME w/ separate header/body column sizing
            }
        }
        else
        {
...
static void O_wrap(sf_node *sfn, int otype)
{
    ...
    if (!fnode)
    {
        fprintf(stderr, "Seriously crashing. Goodbye cruel world.\n");
    }
    endp = sfn->f_cooked;
    while (endp->next)
    {
        endp = endp->next;    /* find end */
    }
    endp->next = fnode;
...
improper:
    ...
    {
badwidth:
        err = "Column widths must be unsigned decimal numbers.";
    }
    if (0)
    {
notmacro:
        err = "Can't set width for a macro (multi-column) format specifier.";
    }
    if (strchr(sfn->sf, '%'))
    {
...
static const char *verify_short_sort(const char *arg)
{
    ...
    const char *walk;
    int tmp;
    if (strspn(arg, all) != strlen(arg))
    {
        return "Bad sorting code.";
    }
    for (i = 256; i--;)
    {
        checkoff[i] = 0;
    ...
    for (;;)
    {
        ...
        switch (tmp)
        {
            ...
            case 'P':
                if (forest_type)
                {
                    return "PPID sort and forest output conflict.";
                }
            /* fall through */
            default:
                if (checkoff[tmp])
                {
                    return "Bad sorting code.";    /* repeated */
                }
...
static const char *parse_O_option(sf_node *sfn)
{
    ...
    switch (sfn->sf_code)
    {
        ...
        case SF_U_O:                                /*** format ***/
            ...
            {
                return "option -O can not follow other format options.";
            }
            err = format_parse(sfn);
            if (err)
            {
            ...
            break;
        case SF_B_O:                                /***  both  ***/
            if (have_gnu_sort || already_parsed_sort)
            {
                err = "Multiple sort options.";
            }
            else
            {
                err = verify_short_sort(sfn->sf);
            }
        ...
        case SF_G_sort:
        case SF_B_m:                 /***  sort  ***/
            if (already_parsed_sort)
            {
                err = "Multiple sort options.";
            }
            else
            {
                err = long_sort_parse(sfn);
            }
            already_parsed_sort = 1;
            break;
        default:                                    /***  junk  ***/
...
int defer_sf_option(const char *arg, int source)
{
    ...
    if (source != SF_U_o)
    {
        return 1;    /* Wonderful! Already non-Unix98. */
    }
    do
    {
        switch (*arg)
        {
            ...
            default:
                ...
                if (dist > 15)
                {
                    return 1;    /* something wrong, sort maybe? */
                }
                ...
                if (!fs)
                {
                    return 1;    /* invalid spec, macro or sort maybe? */
                }
                if (fs->vendor)
                {
                    return 1;    /* Wonderful! Legal non-Unix98 spec. */
                }
        }
    }
    while (*++arg);
...
static const char *generate_sysv_list(void)
{
    ...
    if (format_modifiers & FM_F)
    {
        ...
        if (!(format_modifiers & FM_P))
        {
            PUSH("psr");    /* should be ENG */
        }
    ...
    if (format_flags & FF_Ul)
    {
        ...
        else if (personality & (PER_ZAP_ADDR | PER_IRIX_l))
        {
            PUSH("sgi_p");
        }
        else
    ...
    if ((thread_flags & TF_U_L) && (format_flags & FF_Uf))
    {
        PUSH("nlwp");
    }
    if ((format_flags & (FF_Uf | FF_Ul)) && !(format_modifiers & FM_c))
    {
        PUSH("c");
    }
...
const char *process_sf_options(int localbroken)
{
    ...
    if (personality & PER_BROKEN_o)
    {
        localbroken = 1;
    }
    if (personality & PER_GOOD_o)
    {
        localbroken = 0;
    }
    broken = localbroken;
    ...
    if (format_list)
    {
        printf("Bug: must reset the list first!\n");
    }
    ...
    if (!format_flags && !format_modifiers && !format_list)
    {
        ...
        if (tmp && *tmp)
        {
            ...
            {
                return "Tell procps-feedback@sf.net what you want. (-L/-T, -m/m/H, and $PS_FORMAT)";
            }
            sfn.sf = tmp;
            sfn.f_cooked = NULL;
            err = format_parse(&sfn);
    ...
    if (format_list)
    {
        if (format_flags)
        {
            return "Conflicting format options.";
        }
        if (format_modifiers)
        {
            return "Can't use output modifiers with user-defined output";
        }
        if (thread_flags & TF_must_use)
        {
    ...
    do
    {
        ...
        if (thread_flags & TF_U_L)
        {
            ...
            if (fmt_add_after("SID",  fn))
            {
                goto did_lwp;
            }
            if (fmt_add_after("SESS", fn))
            {
                goto did_lwp;
            }
            if (fmt_add_after("PGID", fn))
            ...
            if (fmt_add_after("PGRP", fn))
            {
                goto did_lwp;
            }
            if (fmt_add_after("PPID", fn))
            {
                goto did_lwp;
            }
