...
#define ENTsz  sizeof(proc_t)
    ...
    else                            // show each thread in a process separately
    {
        ...
        {
            proc_t *ttsk;
            if (unlikely(!(ptsk = readproc(PT, NULL))))
            {
                break;
            }
            ...
            while (curmax < savmax)
            {
                ...
                if (!(ttsk = readtask(PT, ptsk, table[curmax])))
                {
                    break;
                }
                prochlp(ttsk);
        ...
        while (*cp)
        {
            switch (*cp)
            {
                ...
                case 'p':
                    do
                    {
                        if (selection_type && selection_type != 'p')
                        {
                            std_err("conflicting process selection");
                        }
                        selection_type = 'p';
                        if (cp[1])
                        {
                ...
                case 'u':
                    ...
                    {
                        const char *errmsg;
                        if (selection_type /* && selection_type != 'u' */)
                        {
                            std_err("conflicting process selection");
                        }
                        if (cp[1])
        ...
        if (!Rc.mode_altscr || CHKw(w, VISIBLE_tsk))
        {
            ...
            *(s = w->columnhdr) = '\0';
            ...
            for (i = 0; i < w->maxpflgs; i++)
            {
                ...
                if (Screen_cols + 1 < (int)((s - w->columnhdr) + strlen(h)))
                {
                    break;
                }
                s = scat(s, h);
