...
static void arg_parse(int argc, char *argv[])
{
    ...
    if (sel > 1 || select_notty > 1 || select_all > 1 || bsd_c_option > 1 || old_h_option > 1)
    {
        usage();
    }
    if (bsd_c_option)
...
static int stat2proc(int pid)
{
    ...
    if (tty_maj == 24)
    {
        snprintf(P_tty_text, sizeof P_tty_text, "pts/%-3u", tty_min);
    }
    if (P_tty_num == NO_TTY_VALUE)
    {
        memcpy(P_tty_text, "   ?   ", 8);
    }
    if (P_tty_num == DEV_ENCODE(0, 0))
    {
        memcpy(P_tty_text, "console", 8);
    }
#endif
...
static int stat2proc(int pid)
{
    ...
    if (P_tty_num == DEV_ENCODE(12, 255))
    {
        tmp = "consolectl";
    }
    if (tmp)
...
static const char *do_user(void)
{
    ...
    if (P_euid != lastuid)
    {
        ...
        if (p)
        {
            snprintf(buf, sizeof buf, "%-8.8s", p->pw_name);
        }
        else
        {
            snprintf(buf, sizeof buf, "%5d   ", P_euid);
        }
    }
    return buf;
}
...
int main(int argc, char *argv[])
{
    ...
    if (!old_h_option)
    {
        ...
        switch (ps_format)
        {
            ...
            case 0:
                ...
                break;
            case 'l':
                head = "F S   UID   PID  PPID  C PRI  NI ADDR SZ WCHAN    TTY       TIME CMD";
                break;
            case 'f':
                head = "USER       PID  PPID  C STIME   TTY       TIME CMD";
                break;
            case 'j':
                head = "  PID  PGID   SID TTY         TIME CMD";
                break;
            case 'u'|0x80:
                head = "USER       PID %CPU %MEM   VSZ  RSS   TTY   S START     TIME COMMAND";
                break;
            case 'v'|0x80:
                head = "  PID   TTY   S     TIME  MAJFL TRS DRS   RSS %MEM COMMAND";
                break;
            case 'j'|0x80:
                head = " PPID   PID  PGID   SID   TTY   TPGID S   UID     TIME COMMAND";
                break;
            case 'l'|0x80:
                head = "F   UID   PID  PPID PRI  NI   VSZ  RSS WCHAN  S   TTY       TIME COMMAND";
                break;
        }
        printf("%s\n", head);
    }
    ...
    else
    {
        ...
        while ((ent = readdir(dir)))
        {
            ...
            {
                if (!select_notty && P_tty_num == NO_TTY_VALUE)
                {
                    continue;
                }
