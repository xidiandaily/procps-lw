...
static const char *parse_list(const char *arg, const char *(*parse_fn)(char *, sel_union *))
{
    ...
    while (items--)
    {
        ...
        if ((err = (parse_fn)(walk, node->u + items)))
        {
            goto parse_error;
        }
...
static const char *parse_sysv_option(void)
{
    ...
    while (*++flagptr)
    {
        ...
        if (!strchr("aAdefgGlnoptuU", *flagptr))
        {
            not_pure_unix = 1;    // dude, -Z ain't in POSIX
        }
        ...
        switch (*flagptr)
        {
            ...
            case 'C': /* end */
                ...
                if (!arg)
                {
                    return "List of command names must follow -C.";
                }
            ...
            case 'G': /* end */
                ...
                if (!arg)
                {
                    return "List of real groups must follow -G.";
                }
            ...
            case 'J':  // specify list of job IDs in hex (IRIX) -- like HP "-R" maybe?
                ...
                if (!arg)
                {
                    return "List of jobs must follow -J.";
                }
            ...
            case 'O': /* end */
                ...
                if (!arg)
                {
                    return "Format or sort specification must follow -O.";
                }
            ...
            case 'R':    // unknown HP/UX feature, like IRIX "-J" maybe?
                ...
                if (!arg)
                {
                    return "List of PRM groups must follow -R.";
                }
            ...
            case 'U': /* end */
                ...
                if (!arg)
                {
                    return "List of real groups must follow -U.";
                }
            ...
            case 'n': /* end */
                ...
                if (!arg)
                {
                    return "Alternate System.map file must follow -n.";
                }
                namelist_file = arg;
                return NULL; /* can't have any more options */
            case 'o': /* end */
                ...
                if (!arg)
                {
                    return "Format specification must follow -o.";
                }
            ...
            case 'p': /* end */
                ...
                if (!arg)
                {
                    return "List of process IDs must follow -p.";
                }
                err = parse_list(arg, parse_pid);
                if (err)
                {
            ...
            case 's': /* end */
                ...
                if (!arg)
                {
                    return "List of session IDs must follow -s.";
                }
                err = parse_list(arg, parse_pid);
                if (err)
                {
            ...
            case 't': /* end */
                ...
                if (!arg)
                {
                    return "List of terminals (pty, tty...) must follow -t.";
                }
            ...
            case 'u': /* end */
                ...
                if (!arg)
                {
                    return "List of users must follow -u.";
                }
                err = parse_list(arg, parse_uid);
                if (err)
                {
            ...
            case 'z':     /* select by zone */
                ...
                if (!arg)
                {
                    return "List of zones (contexts, labels, whatever?) must follow -z.";
                }
...
static const char *parse_bsd_option(void)
{
    ...
    else
    {
        ...
        else
        {
            if (force_bsd)
            {
                return "2nd chance parse failed, not BSD or SysV.";
            }
        }
    }
    ...
    while (*++flagptr)
    {
        switch (*flagptr)
        {
            ...
            case 'h':
                ...
                if (header_type)
                {
                    return "Only one heading option may be specified.";
                }
                if (personality & PER_BSD_h)
                {
            ...
            case 'k':
                ...
                if (!arg)
                {
                    return "Long sort specification must follow 'k'.";
                }
...
case_Group:
    ...
    if (!arg)
    {
        return "List of real groups must follow --Group.";
    }
    err = parse_list(arg, parse_gid);
    if (err)
    {
...
case_User:
    ...
    if (!arg)
    {
        return "List of real users must follow --User.";
    }
    err = parse_list(arg, parse_uid);
    if (err)
    {
...
case_cumulative:
    ...
    if (s[sl])
    {
        return "Option --cumulative does not take an argument.";
    }
    include_dead_children = 1;
    return NULL;
case_deselect:
    ...
    if (s[sl])
    {
        return "Option --deselect does not take an argument.";
    }
    negate_selection = 1;
    return NULL;
case_no_header:
...
case_noheadings:
    ...
    if (s[sl])
    {
        return "Option --no-heading does not take an argument.";
    }
    if (header_type)
    {
        return "Only one heading option may be specified.";
...
case_headings:
    ...
    if (s[sl])
    {
        return "Option --heading does not take an argument.";
    }
    if (header_type)
    {
        return "Only one heading option may be specified.";
...
case_forest:
    ...
    if (s[sl])
    {
        return "Option --forest does not take an argument.";
    }
    forest_type = 'g';
    return NULL;
case_format:
    ...
    if (!arg)
    {
        return "Format specification must follow --format.";
    }
...
case_group:
    ...
    if (!arg)
    {
        return "List of effective groups must follow --group.";
    }
    err = parse_list(arg, parse_gid);
    if (err)
    {
...
case_pid:
    ...
    if (!arg)
    {
        return "List of process IDs must follow --pid.";
    }
    err = parse_list(arg, parse_pid);
    if (err)
    {
...
case_ppid:
    ...
    if (!arg)
    {
        return "List of process IDs must follow --ppid.";
    }
    err = parse_list(arg, parse_pid);
    if (err)
    {
...
case_sid:
    ...
    if (!arg)
    {
        return "Some sid thing(s) must follow --sid.";
    }
    err = parse_list(arg, parse_pid);
    if (err)
    {
...
case_sort:
    ...
    if (!arg)
    {
        return "Long sort specification must follow --sort.";
    }
    defer_sf_option(arg, SF_G_sort);
    return NULL;
case_tty:
    ...
    if (!arg)
    {
        return "List of ttys must follow --tty.";
    }
    err = parse_list(arg, parse_tty);
    if (err)
    {
...
case_user:
    ...
    if (!arg)
    {
        return "List of effective users must follow --user.";
    }
    err = parse_list(arg, parse_uid);
    if (err)
    {
...
static const char *parse_trailing_pids(void)
{
    ...
    while (i--)
    {
        ...
        if (err)
        {
            return err;    /* the node gets freed with the list */
        }
    }
        ...
        switch (at)
        {
            ...
            case ARG_SYSV:
                if (!force_bsd)   /* else go past case ARG_BSD */
                {
                    ...
                    break;
                case ARG_BSD:
                    if (force_bsd && !(personality & PER_FORCE_BSD))
                    {
                        return "way bad";
                    }
                }
...
static const char *thread_option_check(void)
{
    ...
    if (thread_flags & TF_B_H)
    {
        thread_flags |= (TF_show_proc | TF_loose_tasks);
    }
    if (thread_flags & (TF_B_m | TF_U_m))
    {
        thread_flags |= (TF_show_proc | TF_show_task | TF_show_both);
    }
