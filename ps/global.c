...
static void set_screen_size(void)
{
    ...
    do
    {
        ...
        if (ioctl(STDERR_FILENO, TIOCGWINSZ, &ws) != -1 && ws.ws_col > 0 && ws.ws_row > 0)
        {
            break;
        }
        if (ioctl(STDIN_FILENO,  TIOCGWINSZ, &ws) != -1 && ws.ws_col > 0 && ws.ws_row > 0)
        {
            break;
        }
    ...
    if (columns && *columns)
    {
        ...
        if (!*endptr && (t > 0) && (t < (long)OUTBUF_SIZE))
        {
            screen_cols = (int)t;
        }
    }
...
static const char *set_personality(void)
{
    ...
    if (!s || !*s)
    {
        s = "unknown";    /* "Do The Right Thing[tm]" */
    }
    ...
    if (sl > 15)
    {
        return "Environment specified an unknown personality.";
    }
    strncpy(buf, s, sl);
    buf[sl] = '\0';
