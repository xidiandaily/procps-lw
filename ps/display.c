...
static void signal_handler(int signo)
{
    if (signo == SIGPIPE)
    {
        _exit(0);    /* "ps | head" will cause this */
    }
...
void hex_dump(void *vp)
{
    ...
    while (line--)
    {
        ...
        for (i = 0; i < 16; i++)
        {
            printf(" %2x", (unsigned int)((unsigned char)(charlist[i])));
        }
        printf("\n");
        i = 0;
    }
...
static void arg_show(void)
{
    ...
    while (walk)
    {
        ...
        {
            case SEL_RUID:
                show_uid("RUID", walk->n, walk->u);
                break;
            case SEL_EUID:
                show_uid("EUID", walk->n, walk->u);
                break;
            case SEL_SUID:
                show_uid("SUID", walk->n, walk->u);
                break;
            case SEL_FUID:
                show_uid("FUID", walk->n, walk->u);
                break;
            case SEL_RGID:
                show_gid("RGID", walk->n, walk->u);
                break;
            case SEL_EGID:
                show_gid("EGID", walk->n, walk->u);
                break;
            case SEL_SGID:
                show_gid("SGID", walk->n, walk->u);
                break;
            case SEL_FGID:
                show_gid("FGID", walk->n, walk->u);
                break;
            case SEL_PGRP:
                show_pid("PGRP", walk->n, walk->u);
                break;
            case SEL_PID :
                show_pid("PID ", walk->n, walk->u);
                break;
            case SEL_PPID:
                show_pid("PPID", walk->n, walk->u);
                break;
            case SEL_TTY :
                show_tty("TTY ", walk->n, walk->u);
                break;
            case SEL_SESS:
                show_pid("SESS", walk->n, walk->u);
                break;
            case SEL_COMM:
                show_cmd("COMM", walk->n, walk->u);
                break;
            default:
                printf("Garbage typecode value!\n");
        }
        walk = walk->next;
    }
}
...
static void check_headers(void)
{
    ...
    if (!head_normal)
    {
        lines_to_next_header = -1;    /* how UNIX does --noheader */
    }
}
...
static void lists_and_needs(void)
{
    ...
    if (bsd_e_option)
    {
        if (proc_format_needs & PROC_FILLCOM)
        {
            proc_format_needs |= PROC_FILLENV;
        }
    }
    ...
    {
        needs_for_threads |= PROC_LOOSE_TASKS;
    }
}
...
static int want_this_proc_pcpu(proc_t *buf)
{
    ...
    {
        used_jiffies += (buf->cutime + buf->cstime);
    }

    avail_jiffies = seconds_since_boot * Hertz - buf->start_time;
    if (avail_jiffies)
    {
        pcpu = (used_jiffies << 24) / avail_jiffies;
    }
    ...
    switch (thread_flags & (TF_show_proc | TF_loose_tasks | TF_show_task))
    {
        case TF_show_proc:                   // normal non-thread output
            while (readproc(ptp, &buf))
            {
                ...
                if (buf.cmdline)
                {
                    free((void *)*buf.cmdline);    // ought to reuse
                }
                if (buf.environ)
                {
                    free((void *)*buf.environ);    // ought to reuse
                }
            }
            break;
...
static void prep_forest_sort(void)
{
    ...
    if (!sort_list)      /* assume start time order */
    {
        ...
        if (!incoming)
        {
            fprintf(stderr, "Could not find start_time!\n");
        }
    ...
    if (!incoming)
    {
        fprintf(stderr, "Could not find ppid!\n");
    }
    tmp_list = malloc(sizeof(sort_node));
    tmp_list->reverse = 0;
    tmp_list->typecode = '?'; /* what was this for? */
...
static int compare_two_procs(const void *a, const void *b)
{
    ...
    while (tmp_list)
    {
        ...
        if (result)
        {
            return (tmp_list->reverse) ? -result : result;
        }
...
static void show_tree(const int self, const int n, const int level, const int have_sibling)
{
    ...
    for (;;)  /* look for children */
    {
        ...
        if (processes[i]->ppid == processes[self]->XXXID)
        {
            break;
        }
        i++;
    ...
    for (;;)
    {
        ...
        else if (processes[i + 1]->ppid != self_pid)
        {
            more_children = 0;
        }
...
static void show_forest(const int n)
{
    ...
    while (i--)   /* cover whole array looking for trees */
    {
        ...
        {
            if (processes[j]->XXXID == processes[i]->ppid)
            {
                goto not_root;
            }
        }
...
int main(int argc, char *argv[])
{
    ...
    if (forest_type || sort_list)
    {
        fancy_spew();    /* sort or forest */
    }
    else
    {
        simple_spew();    /* no sort, no forest */
    }
