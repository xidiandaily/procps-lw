...
static int guess_name(char *restrict const buf, unsigned maj, unsigned min)
{
    ...
    switch (maj)
    {
        case   3:      /* /dev/[pt]ty[p-za-o][0-9a-z] is 936 */
            if (tmpmin > 255)
            {
                return 0;    // should never happen; array index protection
            }
        ...
        case 188:
            ...
            break; /* bummer, 9-char */
        case 204:
            if (min >= sizeof low_density_names / sizeof low_density_names[0])
            {
                return 0;
            }
...
unsigned dev_to_tty(char *restrict ret, unsigned chop, dev_t dev_t_dev, int pid, unsigned int flags)
{
    ...
    {
        if (link_name(tmp, MAJOR_OF(dev), MINOR_OF(dev), pid, "tty"))
        {
            goto abbrev;
        }
    }
    if (driver_name(tmp, MAJOR_OF(dev), MINOR_OF(dev)))
    {
        goto abbrev;
    }
    if (link_name(tmp, MAJOR_OF(dev), MINOR_OF(dev), pid, "fd/2"))
    {
        goto abbrev;
    }
    if (guess_name(tmp, MAJOR_OF(dev), MINOR_OF(dev)))
    {
        goto abbrev;
    }
    if (link_name(tmp, MAJOR_OF(dev), MINOR_OF(dev), pid, "fd/255"))
    {
        goto abbrev;
    }
